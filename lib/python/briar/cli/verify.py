...
def verifyParseOptions(inputCommand=None):
    ...
    """
    args = [
        '[reference image/video or directory] [verification image/video or directory] [...]']  # Add the names of arguments here.
    n_args = len(args)
    args = " ".join(args)
...
def verify(options=None, args=None,input_command=None,ret=False):
    ...
    # path_map = pathmap_str2dict(verify_options.path_map)
    for media_file_pair, det_list in zip(zip(reference_media_list, verify_media_list), det_list_list):

        reference_media_file = media_file_pair[0]#pathmap_path2remotepath(media_file_pair[0], path_map)
        verify_media_file = media_file_pair[1]#pathmap_path2remotepath(media_file_pair[1], path_map)
