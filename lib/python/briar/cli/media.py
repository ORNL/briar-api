###################################################################
# Common media handling functions and classes
###################################################################
import briar
import optparse
import os
...
def addMediaOptions(parser):
    """!
    Add options for handling media to the parser. Modifies the parser in place.

    This function adds a group of media-related options to the provided optparse.OptionParser instance.
    These options allow the user to configure various aspects of media handling, such as the maximum size,
    frame range, and context.

    @param parser optparse.OptionParser: A parser to modify in place by adding options.
    """
    media_group = optparse.OptionGroup(parser, "Media Options",
    ...
    # media_group.add_option("-d", "--detections-csv", type="str", dest="detections_csv", default=None,
    #                        help="Save detection data to the file.")
    #
    # media_group.add_option("-a", "--attributes-csv", type="str", dest="attributes_csv", default=None,
    #                        help="Save attributes data to the file.")
    #
    # media_group.add_option("--detect-log", type="str", dest="detect_log", default=None,
    #                        help="A directory for detection images.")
    #
    # media_group.add_option("--face-log", type="str", dest="face_log", default=None,
    #                        help="A directory for faces.")
    #
    # media_group.add_option("-b", "--best", action="store_true", dest="best", default=False,
    #                        help="Detect the 'best' highest scoring face in the image.")
    #
    # media_group.add_option("--detect-thresh", type="float", dest="detect_thresh", default=None,
    #                        help="The threshold for a detection.")
    #
    # media_group.add_option("--min-size", type="int", dest="min_size", default=64,
    #                        help="Faces with a height less that this will be ignored.")
    #
    # media_group.add_option("--attribute-filter", type="str", dest="attribute_filter", default=None,
    #                        help="A comma separated list of filters example: 'Male>0.5'")
    parser.add_option_group(media_group)
...
def hasExtension(f, extension):
    ...
    @param f str: The file name to check.
    @param extension str or list: The extension(s) to check for.
