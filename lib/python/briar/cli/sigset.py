...
def parseSigsetEnrollOptions(inputCommand=None):
    """
    The parseSigsetEnrollOptions function parses command line arguments for the sigset-enroll program.

    :param inputCommand: Pass in the command line arguments
    :return: The options and args
    :doc-author: Joel Brogan, David Bolme, Trelent
    """
    ...
    parser.add_option("--ignore-fisheye", action="store_true", dest="ignore_fisheye", default=False,
                      help="Ignores fisheye lense camera media, with 'M3057-PLVE' in the name")
    ...
    #                   help="Sends across the entire signature set in a single message and waits for enrollmet replies. This allows systems to bulk-enroll on server side.")
    ...
    parser.add_option("--minibatch-size", type="int", dest="minibatch_size", default=-1,
                      help="Explicitly defines the size of a media minibatch (how many peices of media to send to a single service port at once). Default = number_of_service_ports*number_of_services_per_port")
    ...
    parser.add_option("--database-suffix", type="choice", choices=['ADDRESS', 'SERVICE', 'A','S','AS','ADDRESS,SERVICE','ADDRESS+SERVICE'],
                      ...
                      help="Set if the service requires unique database names (acheived via a suffix) for each service running on an address, process, or thread. These suffixes will be determined using <briar.get_process_number>.  If not set, the service will not utilize unique database names.  : Default=None")
    ...
    parser.add_option("--by-subject", action="store_true", dest="by_subject", default=False,
                      help="DEPRICATED. FOR USE WITH LEGACY SYSTEMS ONLY. Does sigset enrollment by subject, and passes only sourceonly media objects.")
    ...
    parser.add_option("--integer-id", action="store_true", dest="integer_id", default=False,
                      help="DEPRICATED. FOR USE WITH LEGACY SYSTEMS ONLY. Indicates that subject ID's will be sent and stored as integers")
...
def sigset_stats(options=None, args=None):
    """
The sigset_stats function parses a sigset file and prints out some statistics about the contents.

:param options: Pass in the command line arguments
:param args: Pass in the command line arguments
:return: The number of unique names, the total media count,
:doc-author: Joel Brogan, David Bolme, Trelent
"""
...
# def initialize_enroll_pools(server_configuration,worker_initializer,counting_queue):
#     global my_pool
#     global proc_number
#     global service_address_number
#     global local_proc_server_port
#     proc_number = briar.get_process_number()
#     thread_number = briar.get_thread_number()
#
#     port_list_number = service_address_number = counting_queue.get()
#
#     port = server_configuration.port_list[port_list_number]
#     local_proc_server_port = port
...
#     print('Initializing main pool number', proc_number, 'for port index ',port_list_number,'and port',port,'connections per port:',connections_per_port)
#     q = BaseManager.Queue()
#     for i in range(connections_per_port):
#         q.put(i+(port_list_number*connections_per_port))
#     my_pool = mp.Pool(connections_per_port, initializer=worker_initializer, initargs=(port, procd_number,thread_number,q,))#,maxtasksperchild=server_configuration.number_of_processes_per_port)
def sigset_enroll(input_command=None):
    """
    The sigset_enroll function is used to enroll a signature set into the Briar system.
    ...
    #We want to split up the full dataframe in to a list of list of dataframes.  This way, each service running on a different port will
